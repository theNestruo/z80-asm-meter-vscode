...
import { mainParserForTimingHintsParsers } from "../parsers/main/MainParser";
...
import { MeterableCollection } from "./AggregatedMeterable";
		...
		switch (config.timing.hints.enabledValue) {
			...
			case "ignoreCommentedOut":
				return meterable
					...
					: this.isCommentedOutSourceCode(sourceCode)
						? undefined
						: new TimingHintedMeterable(new MeterableCollection(), timingHints);
		}
	}

	private static isCommentedOutSourceCode(sourceCode: SourceCode): boolean {
