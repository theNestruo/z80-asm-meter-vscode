...
class Z80InstructionParserImpl implements Z80InstructionParser {
	...
	constructor(instructionSets: string[]) {
		...
		for (const rawData of z80InstructionSet) {
			...
			for (const instruction of originalInstruction.expanded()) {
	...
	parseInstruction(s: SourceCode): Meterable | undefined {
		...
		return this.parseRawInstruction(s.instruction);
	}
...
export class Z80Instruction implements Meterable {
	...
	expanded(): Z80Instruction[] {
		...
		// Expands bit
		if (expandableExpression.startsWith("+8*b")) {
			...
			if (!expandableExpression.startsWith("+8*b+r")) {
				return [
		...
		if (this.instruction.includes("IXp")) {
			return (expandableExpression.startsWith("+8*p"))
		...
		if (this.instruction.includes("IYq")) {
			return (expandableExpression.startsWith("+8*q"))
		...
		if (expandableExpression.startsWith("+p")) {
			return [
		...
		if (expandableExpression.startsWith("+q")) {
			return [
		...
		if (expandableExpression.startsWith("+r")) {
			return [
